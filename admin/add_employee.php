...
function getTodayDateFormatted() {
    // The 'd/m/Y' format string specifies:
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $dateOfJoining = getTodayDateFormatted();
    ...
    // Validation
    if (empty($employeeId) || !preg_match('/^EMP[0-9]{3,}$/', $employeeId)) {
        $errors[] = "Valid Employee ID required (format: EMP001)";
    }
    ...
    if (empty($errors)) {
        try {
            ...
            if ($stmt->fetch()) {
                ...
                $stmt = $db->prepare("
                    INSERT INTO users (employee_id, email, password_hash, role_id, is_verified, is_active)
                    VALUES (?, ?, ?, ?, 1, 1)
                ");
                $stmt->execute([$employeeId, $email, $passwordHash, $roleId]);
                ...
                $stmt = $db->prepare("
                    INSERT INTO employees (
                        ...
                        gender, phone, address, city, state, zip_code,
                        department, designation, date_of_joining, employment_type
                    ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
                ");
                $stmt->execute([
                    $userId, $employeeId, $firstName, $lastName, $dateOfBirth,
                    $gender, $phone, $address, $city, $state, $zipCode,
                ...
                $stmt = $db->prepare("
                    INSERT INTO salary_structure (
                        ...
                        tax_deduction, effective_from
                    ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)
                ");
                $stmt->execute([
                    $empId, $basicSalary, $hra, $conveyanceAllowance,
                    $medicalAllowance, $specialAllowance, $pfDeduction,
                    $taxDeduction, $dateOfJoining
                ]);
...
<body>
    ...
    <div class="main-content">
        ...
        <form method="POST" enctype="multipart/form-data">
            ...
            <div class="card mb-4">
                <div class="card-body">
                    <div class="section-title"><i class="fas fa-lock me-2"></i> Account Information</div>
                    <div class="row">
                        <div class="col-md-6 mb-3">
                ...
                <div class="card-body">
                    ...
                    <div class="row">
                        ...
                        <div class="col-md-3 mb-3">
                            ...
                            <input type="number" name="conveyance_allowance" class="form-control" step="0.01" value="0">
                        </div>
                        <div class="col-md-3 mb-3">
                        ...
                        <div class="col-md-3 mb-3">
                            ...
                            <input type="number" name="pf_deduction" class="form-control" step="0.01" value="0">
                        </div>
                        <div class="col-md-3 mb-3">
                            <label class="form-label">Tax Deduction</label>
                            <input type="number" name="tax_deduction" class="form-control" step="0.01" value="0">
                        </div>
                    </div>
                </div>
            </div>
            ...
            <div class="text-center">
                ...
                <a href="employees.php" class="btn btn-secondary btn-lg px-5">
